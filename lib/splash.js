...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, wide, iframe, gcaptcha_sitekey }){
  let dom_join = dom('p',
    'Join ', dom('b', name),
  ...
  let dom_email = dom('input.form-item type=email name=email placeholder=you@yourdomain.com '
    + (!iframe ? 'autofocus' : ''))
  let dom_captcha = dom(`div class="g-recaptcha" data-sitekey="${gcaptcha_sitekey}"`)
  let dom_coc = coc && dom('.coc',
    dom('label',
  ...
  let dom_loading = dom('button.loading', 'Get my Invite')
  let dom_signin = dom('p.signin',
    'or ',
    dom(`a href=https://${org}.slack.com target=_top`, 'sign in'),
    '.'
  )
  let dom_footer = !iframe && dom('footer',
    'powered by ',
    dom('a href=http://rauchg.com/slackin target=_blank', 'slackin')
  )
  let dom_tail = dom('.tail',
    style({ logo, active, large, iframe, wide }),
    // xxx: single build
    dom('script', `
      data = {};
  ...
  {
    div = dom('.splash',
      dom('form id=invite',dom_captcha,dom_email,dom_loading),
      dom_tail
    )
  }
  return div
...
function style ({ logo, active, large, iframe, wide } = {}){
  var css = dom.style()
  ...
  if (wide) {
    css.add('.splash', { 'width': '100%' })
    css.add('#invite>*', { 'float': 'left', 'margin': '0.5rem 0rem 0rem 1rem'})
    css.add('#invite input', { 'width': '22rem', 'font-size': '1.2rem' })
    css.add('#invite .g-recaptcha', { 'transform': 'scale(1)' })
    css.add('#invite button', { 'width': '17rem' })
    css.add('body, html', { 'background': '#FFFFFF00' })
  }
